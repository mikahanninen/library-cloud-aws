...
Start Textract Document Text Detection Job
    ...
    FOR    ${i}    IN RANGE    50
        ...
        IF    "${response}[JobStatus]" == "SUCCEEDED"    BREAK
        Sleep    1s
    END
...
Check S3 Service
    ...
    #    Log To Console    ${b}
    #END
    ${files}=    List Files    testing-s3-changes
    FOR    ${f}    IN    @{files}
        Log To Console    ${f}
    END
    # testing-s3-changes
    # ExtraArgs={'Metadata': {'mykey': 'myvalue'}}
    &{extras}=    Evaluate    {'Metadata': {'mykey': 'myvalue', 'another': 'withvalue'}}
    ${uploaded}    ${error}=    Upload File
    ...    testing-s3-changes
    ...    ${CURDIR}${/}robot.yaml
    ...    robot.yaml
    ...    ExtraArgs=${extras}
    Log To Console    Uploaded: ${uploaded}
